...
1. **Dockerfile mastery** - Multi-stage builds, optimization, security basics
1. **Docker Compose** - Multi-container apps, networks, volumes
1. **Container concepts** - Namespaces, cgroups, what's actually happening
1. **Registry basics** - Docker Hub, GitHub Container Registry, image tagging

### Intermediate (6-18 months)

1. **Kubernetes fundamentals** - Pods, services, deployments, the core abstractions
1. **kubectl proficiency** - Command-line management, YAML debugging
1. **Helm basics** - Using charts, values overrides, repository management
1. **Persistent storage** - Volumes, StatefulSets, when stateful meets stateless
1. **Security scanning** - Trivy, Grype, fixing CVEs before they bite you

### Advanced (18+ months)

1. **Kubernetes architecture** - Control plane, etcd, scheduler, the guts
1. **Custom resources** - CRDs, operators, extending Kubernetes
1. **Multi-cluster** - Federation, service mesh, the real complexity
1. **Advanced Helm** - Chart development, templating mastery, CI/CD integration
1. **Runtime security** - Falco, admission controllers, zero-trust networking
...
!!! success "Core Principles"
    ...
    1. **Non-root user** - Never run containers as root, it's 2026 for fuck's sake
    1. **Specific tags** - Pin versions, avoid 'latest', reproducibility matters
    1. **Health checks** - Liveness and readiness probes, let K8s help you
    1. **Resource limits** - Set CPU/memory limits always, prevent noisy neighbors
    1. **Secrets management** - External Secrets, Sealed Secrets, not ConfigMaps
    1. **Immutable images** - Build once, deploy everywhere, no SSH into containers
    1. **Security scanning** - Scan in CI, fix CVEs before production
    1. **Network policies** - Default deny, explicit allow, defense in depth
    1. **Monitoring** - Logs to stdout, metrics to Prometheus, traces to Jaeger
...
**Last Updated:** 2026-01-14 **Vibe Check:** 🌍 **Mainstream** - Containers are the standard. Docker for dev, Kubernetes for prod. Everyone's running containers, not everyone's doing it well. YAML fatigue is real but alternatives are worse. Security is still everyone's weakest link. The complexity is worth it, barely.
